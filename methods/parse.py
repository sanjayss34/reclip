...
class Parse(RefMethod):
    ...
    def execute(self, caption: str, env: "Environment") -> Dict[str, Any]:
        ...
        # probs = self._filter(caption, env, root=True)
    ...
    def execute_entity(self, 
        ...
        self.counts["n_rec"] += 1
        # probs = self._filter(ent.text, env, root=root)
        probs = [1, 1]
        head_probs = probs
        ...
        if m1 < self.baseline_threshold * m2:
            ...
            for tokens in ent.superlatives:
                ...
                if sup is not None:
                    ...
                    # superlative_probs = L.meet_reduce(np.maximum(np.eye(sup.shape[0]), 1-intermediate+intermediate*sup))
                    # probs = L.meet(probs, superlative_probs)
                    # # Version 3
