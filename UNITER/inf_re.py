...
def main(opts):
    ...
    if opts.simple_format:
        eval_img_db = DetectFeatPt(opts.img_db, conf_th, model_opts.max_bb,
                                   model_opts.min_bb, num_bb, opts.compressed_db)
    else:
    ...
    for txt_db in txt_dbs:
        ...
        if opts.simple_format:
            eval_txt_db = ReTxtTokJson(txt_db, -1)
...
if __name__ == "__main__":
    ...
    # JSON/PT format
    parser.add_argument('--simple_format', action='store_true')

    parser.add_argument('--cpu', action='store_true')

    args = parser.parse_args()
    print(args.simple_format)
